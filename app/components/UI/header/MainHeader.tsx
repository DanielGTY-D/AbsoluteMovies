...
function Header() {
  ...
  return (
    <div style={{ backgroundImage: `url(\"${imageBaseUrl}\")` }} className='bg-no-repeat bg-cover bg-center w-full h-auto pb-20 relative overflow-hidden after:content-[""] after:bg-black after:w-full after:h-full after:block after:opacity-55 after:absolute after:top-0 after:left-0 after:z-10'>
      {/* {
          ...
          <nav className='flex justify-center md:items-start flex-wrap gap-8 mt-5'>
            <div className='relative group'>
              ...
              <ul className={`absolute bg-rose-800 text-rose-100 rounded-lg overflow-hidden cursor-pointer transform group-hover:scale-100 ${showDropdownMovie ? "scale-100" : "scale-0"} transition-transform duration-500 ease-in-out md:delay-300`}>
                ...
                <li className='text-base text-nowrap p-2 hover:bg-rose-500 transition-colors duration-300'><Link to={"/watchlist/movie/premieres"}>Estrenos</Link></li>
                <li className='text-base text-nowrap p-2 hover:bg-rose-500 transition-colors duration-300'><Link to={"/watchlist/movie/list"}>Movies</Link></li>
              </ul>
            </div>
      ...
      <div className='mt-10 md:mt-20 container mx-auto flex flex-wrap justify-between items-center md:max-w-6xl'>
        ...
        <ul className='flex sm:flex-col mx-auto sm:mx-0 w-[458px] sm:w-fit h-auto sm:h-[457px] px-5 overflow-y-hidden sm:overflow-y-scroll overflow-x-scroll lg:overflow-x-hidden snap-mandatory z-40 relative gap-6 no-scrollbar snap-y scroll-smooth' >
          {upcomingMovies.map((movie, index) => (
            <li key={movie.id} className='snap-center cursor-pointer group relative z-50' onClick={() => setCurrentMovie(index)}>
              <img src={`https://image.tmdb.org/t/p/w200${movie.poster_path}`} alt={movie.title} loading='lazy' className={`group-hover:transform group-hover:scale-105 transition-transform duration-300 max-w-[none] w-45 lg:w-34 h-auto lg:h-46 object-cover rounded-lg  ${index !== currentMovie && "scale-75"}`} />
            </li>
          ))}
        </ul>
